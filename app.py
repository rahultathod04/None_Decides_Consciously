...
st.set_page_config(page_title="EmoDriven", page_icon="🧠", layout="centered")

@st.cache_data
def load_and_train():
    df = pd.read_csv('data/cleaned/student_lifestyle_final.csv')
    features = ['stress_level', 'anxiety_score', 'sleep_hours',
                'mood_rating', 'sleep_debt', 'stress_sleep_ratio', 'state_score']
    X = df[features]
    y = df['at_risk']
...
st.title("🧠 EmoDriven")
st.markdown("*proving that human behavior is driven by emotional state — not free will*")
st.divider()
...
input_data = pd.DataFrame(
    [[stress, anxiety, sleep, mood, sleep_debt, stress_sleep_ratio, state_score]],
    columns=features
)
...
axes[0].hist(df['stress_level'], bins=10, color='lightcoral', edgecolor='white')
axes[0].axvline(x=stress, color='black', linestyle='--', linewidth=2, label=f'you ({stress})')
axes[0].set_title('stress level')
axes[0].legend()

axes[1].hist(df['sleep_hours'], bins=10, color='steelblue', edgecolor='white')
axes[1].axvline(x=sleep, color='black', linestyle='--', linewidth=2, label=f'you ({sleep} hrs)')
